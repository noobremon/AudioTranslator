...
// Allows single or multiple translations per entry. If multiple, fallbackTranslation will join with '/'.
const chineseToEnglishDict: { [key: string]: string | string[] } = {
...
const contextAwareTranslation = (text: string): string => {
  ...
  const phrasePatterns = [
    ...
    { pattern: /我明白了.*/, replacement: 'I understandd' },
...
// Handles both string and string[] for dictionary values
const fallbackTranslation = (text: string, sourceLang: string, targetLang: string): string => {
  if (sourceLang === 'zh' && targetLang === 'en') {
    ...
    if (chineseToEnglishDict.hasOwnProperty(processedText)) {
      const dictValue = chineseToEnglishDict[processedText];
    ...
    for (const [chinese, english] of sortedEntries) {
      if (translated.includes(chinese)) {
        ...
        if (Array.isArray(english)) {
          // TODO: Select the correct translation based on context
          replacement = english[0]; // Default to first meaning
        } else {
          replacement = english;
        }
