...
import { Key, Eye, EyeOff, Save, AlertCircle, CheckCircle, Zap, Globe, Brain, Shield, Mic2 } from 'lucide-react';
    ...
    try {
      ...
      switch (keyType) {
        ...
        case 'elevenLabs': {
          ...
          testResult = elevenLabsResponse.ok;
          break;
        }
        default:
  ...
  return (
    <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-6 border border-white/20 mb-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-2xl font-bold">Enterprise Translation & Voice API Keys</h2>
        {hasAnyKey && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2 text-sm font-semibold text-indigo-300">
            ...
            <span className="text-xs bg-indigo-500/20 px-2 py-1 rounded">Premium Voice</span>
          </label>
          <div className="relative">
            <input
              ...
              placeholder="sk_..."
              className="w-full bg-slate-800/50 border border-slate-600/30 rounded-lg px-4 py-3 pr-20 text-white outline-none focus:border-indigo-500 transition-colors"
            />
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex items-center gap-1">
              {testResults.elevenLabs === 'testing' && <div className="w-4 h-4 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin" />}
              {testResults.elevenLabs === 'success' && <CheckCircle className="w-4 h-4 text-green-400" />}
              {testResults.elevenLabs === 'error' && <AlertCircle className="w-4 h-4 text-red-400" />}
              <button
                onClick={() => testAPIKey('elevenLabs')}
                className="text-xs bg-indigo-500/20 px-2 py-1 rounded hover:bg-indigo-500/30 transition-colors"
              >
                Test
              </button>
              <button
          ...
          <p className="text-xs text-slate-400">Get from: ElevenLabs → Profile → API Key</p>
        </div>
      ...
      <div className="bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-green-500/10 border border-blue-500/30 rounded-lg p-4">
        <h3 className="font-semibold text-blue-300 mb-3 flex items-center gap-2">
          ...
          Enterprise Translation & Voice Features
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-slate-300">
          <ul className="space-y-1">
            <li>• <strong>Multi-API Redundancy:</strong> Automatic failover between 6 translation services</li>
            <li>• <strong>ElevenLabs Premium Voice:</strong> Ultra-realistic AI voice synthesis</li>
          ...
          <ul className="space-y-1">
            ...
            <li>• <strong>Voice Cloning Support:</strong> Custom voice models with ElevenLabs</li>
            <li>• <strong>Professional Audio:</strong> Studio-quality voice output</li>
            <li>• <strong>99.9% Uptime:</strong> Enterprise-grade reliability</li>
          </ul>
        </div>
      </div>
    </div>
